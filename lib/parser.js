...
RdocParser.prototype.readMain = function(toParse, object) {
  ...
  if (matches === null) {
    ...
    object[matches[1]] = this.readMacro("", sliced);
...
RdocParser.prototype.readMacro = function(parsed, toParse) {
  var matches = toParse.match(/(\}|\\([A-Za-z]+)\{)/);
  ...
  if (matches === null) {
    ...
    if (matches[1] === "}") {
      ...
      console.log('new Parsed ' + parsed + toParse.slice(0, index));
      //console.log(toParse.slice(index));
      return this.readMacro(parsed + toParse.slice(0, index), toParse.slice(index));
    }
  }
