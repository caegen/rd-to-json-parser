...
var methodMacroHandler = function() {
  var _generic = this.readLatexLikeArgument();
  ...
  var _class = this.readLatexLikeArgument();
  var _args = '';

  if (/^\s*\(/.test(this._string.slice(this._cursor))) {
    var matches = this._string.slice(this._cursor).match(/^\s*\(([\s\S]*?)\)/);
...
var macroHandlers = {
  ...
  code: function() {
    return '<code>' + this.readLatexLikeArgument() + '</code>';
  },
  link: function() {
    var options = '';
    this._cursor -= 1;
    ...
    var url = '';
    if (arg.indexOf('::') > 0)
      url = arg.replace(/([^"]*)::([^"]*)\(\)/g, '/method-detail?package=$1&method=$2')
    else
      url = arg.replace(/([^"]*)\(\)/g, '/method-detail?package={packageName}&method=$1')
    return `<a href="${url}"> ${arg} </a>`;
  },
  author: function() {
    ...
    switch (true) {
      ...
      case /^\\/.test(toParse):
        ...
        if (escapable.indexOf(next) > -1) {
          ...
          } else if(macro === 'non_function') {
            this.readRLikeArgument(); //ignore it
          }
          else {
...
RdocParser.prototype.handleMacro = function(macroName) {
  ...
  } else if (this._string.charAt(this._cursor - 1) !== '{') { //unknown no arg macro, just leave it
    return '\\' + macroName;
...
RdocParser.prototype.readMacroName = function() {
  var toParse = this._string.slice(this._cursor);
  var matches = toParse.match(/^([A-Za-z0-9_]+)\s*\{?/);
